import{S as x,N as q,a as g,b as A}from"./assets/vendor-CpNM5-39.js";(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const c of a.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&i(c)}).observe(document,{childList:!0,subtree:!0});function t(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function i(s){if(s.ep)return;s.ep=!0;const a=t(s);fetch(s.href,a)}})();const n={openMobileMenuBtn:document.querySelector(".header-mobile-menu"),closeMobileMenuBtn:document.querySelector(".mobile-menu-close"),mobileMenuModal:document.querySelector(".mobile-menu-modal"),booksCategoriesList:document.querySelector(".books-categories-list"),booksCategoriesSelect:document.querySelector(".books-categories-select"),booksCategoriesSelectList:document.querySelector(".books-categories-select-list"),booksList:document.querySelector(".books-list"),booksShowMoreBtn:document.querySelector(".books-item-show-more"),booksCount:document.querySelector(".books-info"),selectBtn:document.querySelector(".books-categories-select-btn"),selectList:document.querySelector(".books-categories-select-list"),dropdownContainer:document.querySelector(".books-categories-select-dropdown")};function m(){n.mobileMenuModal.classList.remove("is-open"),document.body.classList.remove("no-scroll"),n.closeMobileMenuBtn.removeEventListener("click",m),n.openMobileMenuBtn.addEventListener("click",h),document.removeEventListener("keydown",L),n.mobileMenuModal.removeEventListener("click",w),n.openMobileMenuBtn.focus()}function h(){n.mobileMenuModal.classList.add("is-open"),document.body.classList.add("no-scroll"),n.closeMobileMenuBtn.focus(),n.closeMobileMenuBtn.addEventListener("click",m),n.openMobileMenuBtn.removeEventListener("click",h),document.addEventListener("keydown",L),n.mobileMenuModal.addEventListener("click",w)}function L(e){e.key==="Escape"&&m()}function w(e){e.target.classList.contains("mobile-menu-link")&&m()}n.openMobileMenuBtn.addEventListener("click",h);new x(".swiper",{modules:[q],navigation:{nextEl:".hero-button-next",prevEl:".hero-button-prev"}});g.defaults.baseURL="https://books-backend.p.goit.global/books";async function N(){try{return(await g.get("/category-list")).data}catch{return[]}}async function M(e){try{return(await g.get(`/category?category=${e}`)).data}catch{return[]}}async function v(){try{return(await g.get("/top-books")).data}catch{return[]}}async function T(e){try{return(await g.get(`/${e}`)).data}catch{return{}}}function S(e,o){const t=e.map(({_id:i,book_image:s,title:a,author:c,price:d})=>{const p=d?Math.ceil(Number(d)):"";return`
  ...
  `}async function W(e){if(!e)return;const o=await T(e),t=I(o);A.create(t,{onShow:s=>{const a=s.element();document.body.classList.add("mod-open"),a.addEventListener("click",u=>{const y=u.target.closest(".books-accordion-toggle");y&&y.closest(".books-accordion-item").classList.toggle("is-open"),u.target.closest(".books-modal-close-btn")&&s.close(),u.target===a&&s.close()});function c(u){u.key==="Escape"&&s.close()}document.addEventListener("keydown",c);function d(){document.body.classList.remove("mod-open"),document.removeEventListener("keydown",c),console.log("Модалка закрывается (basiclightbox:close)")}s.element().addEventListener("basiclightbox:close",d);const p=s.close;s.close=function(){d(),p.call(s)}}}).show()}function E(){const e=n.booksCount;if(!e)return;const o=Math.min(l,r.length);r.length===0?e.textContent="No books to show":e.textContent=`Showing ${o} of ${r.length}`}function f(e){D(e),U(),E()}function K(){const e=r.slice(l,l+C);S(e,n.booksList),R(),l>=r.length&&B(),E()}n.booksShowMoreBtn.addEventListener("click",K);function b(e){const o=new Set;return e.filter(t=>o.has(t.title)?!1:(o.add(t.title),!0))}async function F(e){try{const o=e.target.closest("li[data-category]");if(!o)return;const t=o.getAttribute("data-category");let i;t==="all"?(i=(await v()).flatMap(a=>a.books),i=b(i)):(i=await M(t),i=b(i)),f(i)}catch(o){console.error("Ошибка при выборе категории:",o)}}async function $(e){try{const o=e.target.value;let t;o==="all"?(t=(await v()).flatMap(s=>s.books),t=b(t)):(t=await M(o),t=b(t)),f(t)}catch(o){console.error("Ошибка при выборе категории через select:",o)}}function G(e){const o=e.target.closest(".books-item-open-modal"),t=e.target.closest(".books-item-image");if(!o&&!t)return;const i=e.target.closest(".books-item");if(!i)return;const s=i.dataset.id;W(s),console.log("Клик по learn more, id книги:",s)}function V(){n.selectList.classList.toggle("is-hidden")}const k=document.querySelector(".books-categories-select-list"),Y=document.querySelector(".books-categories-select-btn-text");k.addEventListener("click",e=>{const o=e.target.closest(".books-categories-select-item");if(!o)return;const t=o.dataset.value,i=o.textContent;Y.textContent=i,k.classList.add("is-hidden"),k.querySelectorAll(".is-active").forEach(s=>s.classList.remove("is-active")),o.classList.add("is-active"),$({target:{value:t}})});j();n.booksCategoriesList.addEventListener("click",F);n.booksCategoriesSelectList.addEventListener("change",$);async function Z(){try{let o=(await v()).flatMap(t=>t.books);o=b(o),f(o)}catch(e){console.error("Ошибка загрузки книг при запуске страницы:",e)}}Z();n.booksList.addEventListener("click",G);n.selectBtn.addEventListener("click",V);
